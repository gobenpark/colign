              ...
              <li key={tabId}>
                ...
                >
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={tabIcons[tabId]} />
                  </svg>
                  {t(tabI18nKeys[tabId])}
                </button>
              </li>
            ...
            <Card className="border-border/50">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-3 pt-2">
                  <Button onClick={() => handleSave("profile")} disabled={saving} className="cursor-pointer">
          ...
          {activeTab === "account" && (
            <>
              <Card className="border-border/50">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="new-password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Min 8 characters" minLength={8} />
                  </div>
                  <div className="flex items-center gap-3 pt-2">
                    <Button onClick={() => handleSave("password")} disabled={saving || !currentPassword || !newPassword} className="cursor-pointer">
                      Update Password
                    </Button>
                    {saved === "password" && <span className="text-sm text-emerald-400">Updated</span>}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-border/50">
                ...
                <CardContent className="space-y-3">
                  <div className="flex items-center justify-between rounded-lg border border-border/50 p-3">
                    ...
                    <Button variant="outline" size="sm" className="cursor-pointer">Connect</Button>
                  </div>
                  <div className="flex items-center justify-between rounded-lg border border-border/50 p-3">
                    <div className="flex items-center gap-3">
              ...
              <Card className="border-destructive/30">
                ...
                <CardContent>
                  <Button variant="outline" className="cursor-pointer border-destructive/50 text-destructive hover:bg-destructive/10">
                    Delete Account
                  </Button>
                </CardContent>
              </Card>
          ...
          {activeTab === "ai" && (
            <Card className="border-border/50">
              <CardHeader>
                ...
                <CardDescription>
                  Configure your own API key for AI features. Without a key, AI features use the platform&apos;s shared quota.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Your key is encrypted and stored securely. It&apos;s only used for your requests.
                  </p>
                </div>
                <div className="flex items-center gap-3 pt-2">
                  <Button onClick={() => handleSave("ai")} disabled={saving} className="cursor-pointer">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Select value={theme} onValueChange={setTheme}>
                    ...
                    <SelectContent>
                      <SelectItem value="dark" className="cursor-pointer">{t("settings.dark")}</SelectItem>
                      <SelectItem value="light" className="cursor-pointer">{t("settings.light")}</SelectItem>
                      <SelectItem value="system" className="cursor-pointer">{t("settings.system")}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={locale} onValueChange={(v) => setLocale(v as "en" | "ko")}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="ko" className="cursor-pointer">한국어</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex items-center gap-3 pt-2">
                  <Button onClick={() => handleSave("appearance")} disabled={saving} className="cursor-pointer">
                    {saving ? t("common.saving") : t("common.save")}
                  </Button>
                  {saved === "appearance" && <span className="text-sm text-emerald-400">{t("common.saved")}</span>}
                </div>
              </CardContent>
            </Card>
          )}
            ...
            <Card className="border-border/50">
              ...
              <CardContent className="space-y-5">
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={emailNotifications} onCheckedChange={setEmailNotifications} className="cursor-pointer" />
                </div>
                <Separator />
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <Switch checked={commentNotifications} onCheckedChange={setCommentNotifications} className="cursor-pointer" />
                </div>
                <Separator />
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">{t("settings.reviewRequests")}</p>
                    <p className="text-xs text-muted-foreground">{t("settings.reviewRequestsDesc")}</p>
                  </div>
                  <Switch checked={reviewNotifications} onCheckedChange={setReviewNotifications} className="cursor-pointer" />
                </div>
                <Separator />
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">{t("settings.stageChanges")}</p>
                    <p className="text-xs text-muted-foreground">{t("settings.stageChangesDesc")}</p>
                  </div>
                  <Switch checked={stageNotifications} onCheckedChange={setStageNotifications} className="cursor-pointer" />
                </div>
                <div className="flex items-center gap-3 pt-2">
                  <Button onClick={() => handleSave("notifications")} disabled={saving} className="cursor-pointer">
                    {saving ? t("common.saving") : t("common.save")}
                  </Button>
                  {saved === "notifications" && <span className="text-sm text-emerald-400">{t("common.saved")}</span>}
                </div>
              </CardContent>
            </Card>
          )}
